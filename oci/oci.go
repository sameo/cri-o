...
func New(runtimePath string, privilegedRuntimePath string, conmonPath string, conmonEnv []string, cgroupManager string) (*Runtime, error) {
	r := &Runtime{
		name:           filepath.Base(runtimePath),
		path:           runtimePath,
		privilegedPath: privilegedRuntimePath,
...
func (r *Runtime) Path(c *Container) string {
	if c.privileged == true && r.privilegedPath != "" {
